(function () {
    ...
    function render() {
        ...
        procs.forEach(function (p) {
            var badge = '<span class="br-status pending">已停止</span>';
            if (p.status === 'running') badge = '<span class="br-status ok">运行中</span>';
            else if (p.status === 'error') badge = '<span class="br-status err">错误</span>';
            ...
            var errHtml = (p.status === 'error' && p.error) ? '<div class="banner banner-err" style="margin-top:0.5rem;">' + App.escapeHtml(p.error) + '</div>' : '';
            ...
            html += '<div class="hs-card" data-id="' + p.id + '">' +
                '<div class="hs-card-head">' +
                '<div class="hs-card-title">' +
                '<span class="hs-name">' + App.escapeHtml(p.name) + '</span>' +
    ...
    function renderEnvEditor(envs) {
        ...
        list.forEach(function (e, i) {
            ...
            row.innerHTML =
                '<input type="text" class="br-input proc-env-name" placeholder="KEY" value="' + App.escapeHtml(e.name || '') + '">' +
                '<span class="hs-m-arrow">=</span>' +
                '<input type="text" class="br-input proc-env-val" placeholder="value" value="' + App.escapeHtml(e.value || '') + '">' +
    ...
    document.getElementById('f-env-add').addEventListener('click', function () {
        ...
        row.className = 'proc-env-row';
        row.innerHTML =
            '<input type="text" class="br-input proc-env-name" placeholder="KEY">' +
            '<span class="hs-m-arrow">=</span>' +
            '<input type="text" class="br-input proc-env-val" placeholder="value">' +
    ...
    function pollLogs() {
        ...
        Api.localTools.procs.logs(logId, logSeq, 500).then(function (data) {
            ...
            if (data.truncated && body.children.length === 0) {
                body.innerHTML = '<div class="br-empty">（已截断，缓冲区溢出）</div>';
            }
            var html = '';
            ...
            if (data.running) { st.textContent = '运行中'; st.className = 'br-status ok'; }
            else { st.textContent = '已停止'; st.className = 'br-status pending'; }
            // auto scroll
